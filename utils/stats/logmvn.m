...
if size(Mu,1) == 1 && ndims(Sigma) == 2 
    % single mean and single covariance
    Mu = repmat(Mu, size(X,1), 1);
    y = loggausspdf(X, Mu, Sigma);
elseif size(Mu,1) == size(X,1) && ndims(Sigma) == 2 && all(size(Sigma) == size(X,2))
    % multiple mean and single covariance
    y = loggausspdf(X, Mu, Sigma);
elseif size(Mu,1) == size(X,1) && size(Sigma,3) == size(X,1) 
    % multiple means and multiple covariances
...
d = size(X,1);
if isdiag(Sigma)
    log_pdf = zeros(size(X));
    for i = 1:d
        log_pdf(i,:) = -log(2*pi)/2-log(Sigma(i,i))/2-(X(i,:)-mu(i,:)).^2/(2*Sigma(i,i));
    end
    y = sum(log_pdf,1);
else
    X = bsxfun(@minus,X,mu);
