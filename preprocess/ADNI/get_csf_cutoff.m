...
% diff = comp1 - comp2;
% diff_l([1,1:end-1]) diff_r([2:end,end])
...
title({'ADNI CSF A\beta_{1-42} distribution',''},'fontweight','normal');
...
h_line = line([cutoff_value, cutoff_value], [0, 0.001], ...
...
% cutoff_value = 880;
line([cutoff_value, cutoff_value], [0, max([histcounts(ABETA_CN, edges), ...
...
xlabel('CSF A\beta_{1-42} (pg/ml)');
ylabel('Count');
title({'ADNI CSF A\beta_{1-42} distribution per clinical diagnosis',''},'fontweight','normal');
