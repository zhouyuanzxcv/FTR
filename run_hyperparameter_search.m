...
if choose_search_max
    save_file = [save_path, 'traj_data_list_run_search_max.mat']; % for search_max
    options.deconv = 'search_max';
else
    save_file = [save_path, 'traj_data_list_run_min_max.mat']; % for min_max
...
title({title_str, sprintf('Best MAE %.4f at (%g, %g)', M, lambdas(row_idx), mus(col_idx))}, ...
    'fontweight','normal');
end
...
search_result_file = 'coarse_grid_search_run_search_max';
lambdas = 10.^(-4:2);
mus = 10.^(-4:2);
...
search_result_file = 'fine_grid_search_run_search_max';
lambdas = 10 * 2.^(-3:3);
mus = 0.01 * 2.^(-3:3);
